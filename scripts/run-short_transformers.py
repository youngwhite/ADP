...
from src.traintest import set_random_seed, train_epoch_amp, validate
...
class ShortASTC(ASTForAudioClassification):
    ...
    def analyse_layers(valset: Dataset, model: nn.Module, key: str='fbank', device: str = "cpu"):
...
if __name__ == '__main__':
    ...
    for fold in range(1, folds+1):
        ...
        model.fit(trainset, valset, batch_size=48, lr=1e-5, epochs=100)
        ...
        # 测试剪枝后的推理
        model.fit(trainset, valset, batch_size=32, lr=1e-5, epochs=30)
        # torch.save(model.bstate, f"fold_{fold}_{str(model.param)}_{str(model.acc)}.pth")
